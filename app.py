...
class UiWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.max_lengths = {col: self.query_results_df[col].astype(str).map(len).max() for col in
                            self.query_results_df.columns}
    ...
    def select_file_or_folder(self):
        ...
        file_path, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Select File or Folder", "",
                                                             "All Files (*);;Folders", options=options)
        if file_path:
